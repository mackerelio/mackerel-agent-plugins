...
opcache_hit_rate:15`
	...
	assert.Equal(t, stat["num_cached_keys"], 2)
	assert.Equal(t, stat["max_cached_keys"], 7963)
	assert.Equal(t, stat["hits"], 123)
	assert.Equal(t, stat["oom_restarts"], 0)
	assert.Equal(t, stat["hash_restarts"], 0)
	assert.Equal(t, stat["manual_restarts"], 0)
	assert.Equal(t, stat["misses"], 53)
	assert.Equal(t, stat["blacklist_misses"], 12)
	assert.Equal(t, stat["blacklist_miss_ratio"], 10)
	assert.Equal(t, stat["opcache_hit_rate"], 15)
}
